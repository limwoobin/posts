...
1과 0은 반드시 연속되게어야 합니다. `11100111. 00111100...` 이런식으로는 나타낼 수 없습니다.
...
즉, `2^8 - 2 = 254` 의 범위를 가지게되어 `192.168.10.1 - 192.168.10.254`가 호스트의 범위가 되고 개수는 `2^8 - 2 = 254` 가 됩니다.  

2를 빼는 이유는 위에서 네트워크 주소와 브로드캐스트 주소는 재외한다고 말씀드렸습니다.  

네트워크 주소는 해당 ip의 첫번째 주소, 브로드캐스트 주소는 마지막 주소이니 
여기서는 `네트워크 주소: 192.168.10.0`, `브로드캐스트 주소: 192.168.10.255` 가 되게 됩니다.
...
그리고 호스트 영역은 __<span style='color: skyblue'>00000</span>__ 입니다. 저 부분을 게산하면 호스트의 범위는 `2^5 - 2 = 30` 이므로 `150.115.120.1 - 150.115.120.30` 이고 개수는 30개가 됩니다.  
