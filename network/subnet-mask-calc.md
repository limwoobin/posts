...
즉, `192.168.10.0/24` 의 사용가능한 네트워크 영역은 `192.168.10.0` 이 되고 호스트의 범위는 `192.168.10.1 - 192.168.10.254`, 개수는 `2^8 - 2 = 254` 가 됩니다.  
2를 빼는 이유는 위에서 네트워크 주소와 브로드캐스트 주소는 재외한다고 말씀드렸습니다.  

네트워크 주소는 해당 ip의 첫번째 주소, 브로드캐스트 주소는 마지막 주소라고 했습니다.  
그렇다면 여기서는 `네트워크 주소: 192.168.10.0`, `브로드캐스트 주소: 192.168.10.255` 가 되게 됩니다.
...
위 값을 게산하면 네트워크 영역은 `10010110. 01110011. 01111000. 00000000 (150.115.120.0)` 가 됩니다.  
그리고 호스트 영역은 __<span style='color: skyblue'>00000</span>__ 입니다. 저 부분을 게산하면 호스트의 범위는 `150.115.120.1 - 150.115.120.30` 이고 호스트의 개수는 `2^5 - 2 = 30` 이 됩니다.  
네트워크 주소는 `150.115.120.1`, 브로드캐스트 주소는 `150.115.120.31` 이 됩니다.
