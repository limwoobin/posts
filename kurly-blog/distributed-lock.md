...
분산락 구현은 `Zookeeper, Redis, Mysql` 등을 이용하면 모두 구현할 수 있습니다.  
...
그리고 레디스 라이브러리로 `Redisson`를 선택한 이유는 `Lettuce` 와 비교했을때 락 획득 방식, 타임아웃 설정 등 많은 차이들이 있습니다.  
자세한 차이가 궁금하시다면 다음 레퍼런스를 참고해보시길 추천드립니다.
